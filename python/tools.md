...
- **Линтер** (`ruff check`, `flake8`) — находит ошибки и проблемы со стилем, может не всё исправить автоматически
...
Бескомпромиссный ("opinionated") форматтер — почти не имеет настроек. Единый стиль для всех проектов. Устраняет споры о стиле кода в команде.

---

## Что такое mypy и Pyright

Статические проверщики типов. Анализируют type hints и находят ошибки типов без запуска кода.

- **mypy** — оригинальный checker, `mypy --strict` для строгого режима
- **Pyright** (от Microsoft) — быстрее mypy, используется в Pylance для VSCode
