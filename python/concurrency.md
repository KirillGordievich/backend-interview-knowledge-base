...
Daemon thread — поток, который не удерживает Python-программу от завершения. Когда основной поток и все non-daemon потоки завершатся, Python завершит процесс, даже если daemon threads ещё работают.
...
print("main finished")
# программа завершится, несмотря на бесконечный цикл в worker
```

Хорошо подходит для фоновых задач, которые не критично корректно завершить — периодический мониторинг, обновление кэша, heartbeat, фоновые проверки.

Не стоит использовать для операций, которые обязательно должны быть завершены — запись в БД, отправка платежей, сохранение файлов. Процесс может завершиться, и daemon thread будет просто убит на полуслове.

Обычный (non-daemon) поток должен завершиться, прежде чем Python-процесс сможет завершиться. Daemon — нет. Установить флаг нужно до вызова `start()`.

То же самое работает для процессов из `multiprocessing` — `Process(target=worker, daemon=True)`. Логика идентична: daemon-процесс завершится вместе с родительским процессом.
