...
**Где полезен:** для фоновых задач, которые не критично корректно завершить — периодический мониторинг, обновление кэша, heartbeat, фоновые проверки.

**Где НЕ использовать:** для операций, которые обязательно должны быть завершены — запись в БД, отправка платежей, сохранение файлов. Процесс может завершиться, и daemon thread будет просто остановлен на полуслове.

**Чем отличается от обычного потока:** обычный (non-daemon) поток должен завершиться, прежде чем Python-процесс сможет завершиться. Daemon — нет. Установить флаг нужно до `start()`.
