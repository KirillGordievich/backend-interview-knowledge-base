...
Стандартный цикл событий asyncio использует модуль `selectors` (`epoll` на Linux, `kqueue` на macOS). Для более высокой производительности можно заменить его на `uvloop` (обёртка над `libuv`).
...
### Что такое async comprehensions?
...
Асинхронное — позволяет итерироваться по async-генератору. `async for` внутри себя неявно вызывает `__anext__()` на каждой итерации — поэтому `x` уже готовое значение, а не корутина, и дополнительный `await` не нужен:

```python
async def stream_rows():
    async with db.cursor() as cursor:
        await cursor.execute("SELECT id FROM users")
        async for row in cursor:  # каждый row приходит асинхронно
            yield row[0]

async def main():
    ids = [row_id async for row_id in stream_rows()]
```

Полезно, когда источник данных асинхронный — стриминг из БД, чтение из очереди, HTTP-стрим.

При этом `async for` не означает «параллельно» — элементы всё равно получаются последовательно, просто итератор сам по себе асинхронный.

### Чем `[await foo(x) for x in items]` отличается от `asyncio.gather`?

`await` внутри comprehension выполняет операции последовательно — следующий `await` начинается только после завершения предыдущего:

```python
# foo(1) запускается → ждём результат → foo(2) запускается → ждём → foo(3)...
# общее время = сумма всех операций
result = [await foo(x) for x in items]
```

`asyncio.gather` создаёт все задачи сразу и ждёт их вместе — они выполняются конкурентно:

```python
# foo(1), foo(2), foo(3) все запускаются сразу
# общее время ≈ время самой долгой операции
result = await asyncio.gather(*(foo(x) for x in items))
```

Если `foo` — это HTTP-запрос на 100мс, то для 10 элементов:
- comprehension с `await`: ~1000мс (10 × 100мс)
- `gather`: ~100мс (все параллельно)

Для независимых I/O-операций `gather` значительно быстрее. Но если операции зависят друг от друга — нужен последовательный вариант.
