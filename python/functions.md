...
### Что такое `/` и `*` в параметрах функции

В сигнатуре функции `/` и `*` — это не параметры, а **разделители**, которые ограничивают способ передачи аргументов.

**`/` (slash)** — всё что слева от него можно передавать **только по позиции**, нельзя по имени. Появился в Python 3.8. Зачем: позволяет автору функции свободно переименовывать параметры, не ломая вызывающий код. Также используется в стандартной библиотеке — например, `len(obj, /)` не даст написать `len(obj=my_list)`.
...
div(10, 3)      # OK
div(a=10, b=3)  # TypeError — a и b positional-only
```

**`*` (звёздочка без имени)** — всё что справа от неё можно передавать **только по имени**. Это заставляет вызывающий код явно указывать имена аргументов, что делает вызов более читаемым.
...
connect('localhost', 5432)            # TypeError — только по имени
```

### Все зоны вместе

Когда `/` и `*` используются одновременно, параметры делятся на три зоны:

```python
def f(a, /, b, *, c):
    pass
#    ^       ^      ^
#    |       |      └── только по имени (keyword-only)
#    |       └── как угодно (по позиции или по имени)
#    └── только по позиции (positional-only)
```

```python
f(1, 2, c=3)     # OK — a по позиции, b по позиции, c по имени
f(1, b=2, c=3)   # OK — a по позиции, b по имени, c по имени
f(a=1, 2, c=3)   # TypeError — a можно только по позиции
f(1, 2, 3)       # TypeError — c можно только по имени
```
...
2. Обычные — positional or keyword (между `/` и `*`)
3. `*args` — собирает оставшиеся позиционные аргументы в кортеж
4. Keyword-only (после `*` или `*args`)
5. `**kwargs` — собирает оставшиеся именованные аргументы в словарь
