...
### Как сделать отложенную инициализацию через property?
...
Полезно когда создание ресурса дорогое (`DatabaseConnection`, `HttpClient`), но конкретному объекту он может вообще не понадобиться. `@property` при этом позволяет обращаться к нему как к обычному атрибуту — `service.client` вместо `service.get_client()`.
...
| JSON serialization | нет | да (`.model_dump()`) |
| Зависимости | стандартная библиотека | внешняя библиотека |
| Overhead | минимальный | есть (validation при создании) |
...
Для внешних данных (HTTP request, конфиг, message queue) обычно выбирают Pydantic. Для внутренних моделей и domain objects — `dataclass` проще и достаточно. Но это не жёсткое правило, зависит от архитектуры.
...
В ООП система моделируется через объекты, которые инкапсулируют состояние и операции над ним:
...
class Cart:
    ...
    def add(self, item):
        self.items.append(item)  # мутирует внутреннее состояние
```

В функциональном подходе данные и операции над ними разделяются. Функции не изменяют входные данные, а возвращают новые:

```python
def add_item(items, item):
    return [*items, item]  # оригинальный список не изменяется
```

Часто говорят что главное различие — mutable vs immutable state, но это упрощение. Хорошее ООП тоже может использовать immutable-объекты, а FP-программы тоже имеют состояние — просто управляют им иначе. Это скорее разные способы организации кода и мышления.

Python — мультипарадигменный язык. Поддерживает оба подхода: полноценное ООП (классы, наследование, инкапсуляция) и элементы FP (функции первого класса, замыкания, `map`/`filter`, `functools`). На практике Python-код обычно комбинирует оба стиля.
