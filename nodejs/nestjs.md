...
NestJS — фреймворк для Node.js на TypeScript. Вдохновлён Angular: модульная архитектура, декораторы, DI из коробки. Под капотом использует Express по умолчанию, но можно переключить на Fastify — интерфейс NestJS остаётся тем же.
...
Слоистая архитектура (Layered Architecture) — это общий паттерн проектирования, не специфичный для NestJS. Его используют в любых бэкенд-фреймворках (Spring, Django, и т.д.). Но NestJS подталкивает к нему своей структурой: `nest g resource` сразу генерирует Controller + Service + DTO, а модули инкапсулируют слои.

Суть — каждый слой отвечает за своё и зависит только от слоя ниже:
...
Controller не знает про БД. Service не знает про HTTP. Это даёт возможность заменить любой слой независимо — например, поменять ORM с TypeORM на Prisma, не трогая контроллеры.

---

## Архитектура: модули

NestJS организует код через модули — каждый модуль объединяет контроллеры, сервисы и другие провайдеры, связанные с одной фичей. Корневой `AppModule` импортирует остальные модули и собирает приложение в единое целое.
...
Модуль — класс с декоратором `@Module()`, который описывает что входит в этот модуль: контроллеры, провайдеры, импорты других модулей и что экспортируется наружу.
...
Контроллер обрабатывает входящие HTTP-запросы. Декораторы (`@Get`, `@Post`, `@Param`, `@Body`) определяют маршруты и откуда брать данные. Сам контроллер не содержит бизнес-логику — он делегирует работу сервису.
...
А в модуле решаем, какую реализацию подставить. Чаще всего используется `useExisting` — это основной способ привязать токен к конкретной реализации. Остальные варианты (`useClass`, `useFactory`, `useValue`) нужны реже, под специфичные задачи:

```ts
// useExisting — основной способ. Алиас на уже зарегистрированный провайдер (один экземпляр на оба токена)
@Module({
    providers: [
