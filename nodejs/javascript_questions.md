...
Объявления `var` и `function declaration` поднимаются в начало области видимости. `var` поднимается как `undefined`, функция — целиком, следовательно мы можем обращаться к функциям и к var переменным до их объявления в коде без ошибок. `let`/`const` переменные нельзя использовать до объявления, потому что обращение до инициализации вызовет `ReferenceError`.
...
this — это специальное значение внутри функции, которое указывает на контекст вызова этой функции. Важно: для обычных функций this определяется в момент вызова, а не в момент объявления функции.
