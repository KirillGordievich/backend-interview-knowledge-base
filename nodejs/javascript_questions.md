...
**Q: Почему `typeof null === "object"`?**

Историческая ошибка в первой реализации JS. В первой версии тип определялся по битовой метке, а у `null` (нулевой указатель) метка совпала с `object`. Исправить нельзя — сломает существующий код.
