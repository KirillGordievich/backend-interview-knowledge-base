...
**Функции — это тоже объекты?** Да, функции — это callable objects (вызываемые объекты). У них есть свойства и методы как у обычных объектов:
...
// можно добавлять свои свойства
greet.counter = 0;
greet.counter++;  // работает
```

`typeof` возвращает `"function"` — это просто удобное исключение в спецификации, чтобы отличать вызываемые объекты от обычных. Но `instanceof Object` подтверждает что функция — объект:
...
`this` — это ключевое слово, которое ссылается на объект, в контексте которого была вызвана функция. В отличие от большинства языков (Java, C#, Python), где `this`/`self` всегда указывает на экземпляр класса, в JavaScript `this` определяется **в момент вызова**, а не в момент объявления. Одна и та же функция может иметь разный `this` в зависимости от того, как её вызвали.
...
## Что будет при переполнении стека (stack overflow)

JavaScript использует **call stack** — стек вызовов функций. Каждый вызов функции добавляет фрейм в стек, а при возврате — убирает. Стек имеет ограниченный размер (зависит от движка, обычно ~10-25 тысяч фреймов).

При бесконечной рекурсии стек переполняется и выбрасывается `RangeError: Maximum call stack size exceeded`:
...
Ошибка **не ловится** на уровне event loop — это синхронное исключение, но его можно поймать через `try/catch`:
...
**Частые причины в реальном коде:**
- Рекурсия без базового случая (забыли условие выхода)
- Два геттера/сеттера, которые вызывают друг друга
- Сериализация объектов с циклическими ссылками — `JSON.stringify` обычно кидает `TypeError: Converting circular structure to JSON`, но если сериализация происходит внутри библиотеки (например, NestJS Logger), она может уйти в рекурсию и упасть именно с `RangeError`. Типичный кейс — spread ответа от Axios (`...response`), который содержит `request → socket → connection → ...` с циклами

**Как избежать:** если рекурсия может быть глубокой — переписать на итеративный подход (цикл + свой стек), либо разбить на асинхронные вызовы через `setTimeout`/`setImmediate`, чтобы каждый шаг рекурсии начинался с чистого стека.
...
// Ещё хуже — «мёртвая» запись, к которой невозможно обратиться:
let key = { heavy: new Array(1_000_000) };
map.set(key, "important data");
key = null;
// Ссылка на ключ потеряна, но Map всё ещё хранит и ключ, и значение.
// map.size === 2, память занята, а достать "important data" уже невозможно —
// мы не знаем по какому ключу она лежит. Классическая утечка памяти.
...
> Все сложности указаны для V8 (Node.js, Chrome). Реализация структур данных в V8 может отличаться от спецификации — спецификация не требует конкретных алгоритмов, но на практике все современные движки используют описанные ниже подходы.

### Array

Массивы в V8 бывают двух видов: **packed** (плотный — все индексы заполнены) и **holey** (разреженный — есть пропуски). Packed-массивы хранятся как непрерывный блок памяти, holey — как хеш-таблица. Переход из packed в holey происходит, например, при `arr[1000] = 1` на массиве длины 3, и обратно не конвертируется.

| Операция | Сложность | Примечание |
|---|---|---|
| `arr[i]` | O(1) | прямой доступ по индексу |
| `arr[i] = x` | O(1) | |
| `arr.push(x)` | O(1) амортизированная | иногда O(n) при росте буфера |
| `arr.pop()` | O(1) | удаление с конца |
| `arr.shift()` | O(n) | сдвиг всех элементов влево |
| `arr.unshift(x)` | O(n) | сдвиг всех элементов вправо |
| `arr.splice(i, k)` | O(n) | удаление/вставка в середине |
| `arr.slice(i, j)` | O(k) | k = j - i (копирование) |
| `arr.indexOf(x)` | O(n) | линейный поиск |
| `arr.includes(x)` | O(n) | линейный поиск |
| `arr.find(fn)` | O(n) | |
| `arr.map(fn)` / `arr.filter(fn)` | O(n) | создаёт новый массив |
| `arr.reduce(fn)` | O(n) | |
| `arr.sort()` | O(n log n) | Timsort в V8 |
| `arr.reverse()` | O(n) | |
| `arr.concat(arr2)` | O(n + m) | новый массив |
| `arr.length` | O(1) | |

```js
const arr = Array.from({ length: 1_000_000 }, (_, i) => i);
arr.unshift(-1);   // O(n) — сдвигает миллион элементов
arr.push(-1);      // O(1) — добавляет в конец
```

**Когда Array плохо подходит:**
- Частые вставки/удаления в начало → рассмотреть связный список или пересмотреть алгоритм
- Поиск по значению → `Set` или `Map`
- Очередь (FIFO) → для критичных к производительности случаев использовать библиотеку (например, `denque`)

---

### Object

Обычные объекты в V8 оптимизированы через **hidden classes** (скрытые классы). Пока структура объекта стабильна (одни и те же ключи в одном порядке), доступ по ключу — практически O(1). Если объект часто мутируется (добавляются/удаляются ключи), V8 переводит его в **dictionary mode** (хеш-таблица).

| Операция | Сложность | Примечание |
|---|---|---|
| `obj[key]` / `obj.key` | O(1) | через hidden class или хеш-таблицу |
| `obj[key] = v` | O(1) | |
| `delete obj[key]` | O(1) | но переводит в dictionary mode — замедляет дальнейший доступ |
| `key in obj` | O(1) | |
| `Object.keys(obj)` | O(n) | создаёт массив ключей |
| `Object.values(obj)` | O(n) | |
| `Object.entries(obj)` | O(n) | |
| `Object.assign(target, src)` | O(n) | n = количество свойств |
...
Реализована как хеш-таблица. В отличие от `Object`, ключом может быть любое значение, и `Map` оптимизирована для частого добавления/удаления записей (не теряет производительность как Object в dictionary mode).

| Операция | Сложность | Примечание |
|---|---|---|
| `map.get(key)` | O(1) среднее | |
| `map.set(key, v)` | O(1) среднее | |
| `map.has(key)` | O(1) среднее | |
| `map.delete(key)` | O(1) среднее | |
| `map.size` | O(1) | |
| `map.clear()` | O(1) | |
| Итерация (`for...of`) | O(n) | порядок вставки |

---

### Set

Та же хеш-таблица, что и `Map`, но хранит только ключи.

| Операция | Сложность | Примечание |
|---|---|---|
| `set.has(x)` | O(1) среднее | |
| `set.add(x)` | O(1) среднее | |
| `set.delete(x)` | O(1) среднее | |
| `set.size` | O(1) | |
| `set.clear()` | O(1) | |
| Итерация | O(n) | порядок вставки |

---

### String

Строки неизменяемы. V8 хранит их в разных внутренних представлениях (SeqString, ConsString, SlicedString) в зависимости от того, как строка была создана.
...
| `s.slice(i, j)` | O(k) | k = j - i |
| `s.indexOf(sub)` | O(n·m) в худшем случае | |
| `s.includes(sub)` | O(n·m) в худшем случае | |
| `s.split(sep)` | O(n) | |
| `s.replace(a, b)` | O(n) | |
| Конкатенация в цикле (`+=`) | O(n²) суммарно | анти-паттерн |
| `arr.join(sep)` | O(n) | правильный способ |
