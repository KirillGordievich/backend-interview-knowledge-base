...
Принцип про то, что не должно быть **объекта-бога** — класса, который знает всё и делает всё. Каждый класс должен отвечать за одну конкретную вещь.

Формально это звучит как «одна причина для изменения»: если класс меняется по двум разным причинам (например, изменилась бизнес-логика И изменился формат email), значит он делает две вещи и их надо разделить.
