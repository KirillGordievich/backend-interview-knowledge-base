...
## Как устроен синтаксис OVER

Оконная функция определяется через `OVER(...)`. Внутри можно указать разбиение на группы, порядок сортировки и рамку:
...
## Какие бывают ранжирующие функции
...
## Какие бывают функции смещения

Функции смещения позволяют обращаться к значениям из соседних строк или границ окна — без JOIN и подзапросов:
...
## Как использовать агрегатные функции как оконные

Любую агрегатную функцию (`SUM`, `AVG`, `COUNT`, `MAX`, `MIN`) можно использовать с `OVER` — тогда она вычисляется в рамках окна, не схлопывая строки:
...
## Что такое рамка окна (Frame) и зачем она нужна
...
## Какие задачи решают оконные функции на практике

Несколько типичных паттернов, которые часто встречаются на собеседованиях и в реальных задачах:
