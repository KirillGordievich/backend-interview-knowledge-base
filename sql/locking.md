...
## Какие уровни блокировок существуют

### Что такое строчные блокировки (row-level)

Блокирует только конкретные строки — минимальное влияние на конкурентность. PostgreSQL использует по умолчанию.

### Что такое табличные блокировки (table-level)
...
### Что такое advisory locks и зачем они нужны
...
## Как работают блокировки строк

### Какие режимы строчных блокировок есть в PostgreSQL

PostgreSQL поддерживает четыре режима строчных блокировок — от самого слабого до самого строгого:
...
## Как мониторить блокировки в PostgreSQL

Для диагностики блокировок в PostgreSQL есть несколько системных представлений. `pg_locks` показывает все текущие блокировки, а `pg_stat_activity` — какие запросы их держат:
...
Если блокирующую транзакцию нужно снять вручную:

```sql
-- Мягкое завершение (SIGINT) — отменяет текущий запрос
SELECT pg_cancel_backend(pid);
-- Жёсткое завершение (SIGTERM) — убивает соединение
